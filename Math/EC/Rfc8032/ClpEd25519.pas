...
{$SCOPEDENUMS OFF}
  ...
  class procedure ScalarMult(const k: TCryptoLibByteArray; var p: TPointAffine;
    var r: TPointAccum); static;
 function SSHVerify(const sig: TCryptoLibByteArray; sigOff: Int32;
      const pk: TCryptoLibByteArray; pkOff: Int32;
      const ctx: TCryptoLibCustomByteArrayBuffer; phflag: Byte;
      const m: TCryptoLibByteArray; mOff, mLen: Int32): Boolean;
...
//PK = K_S, ctx = a random 0~255 length random number,
//phflag = 1 for CTX mode, 2 for PH mode
//m = exchange Hash of SSH
//sig = 64 bytes remote signature

function TEd25519.SSHVerify(const sig: TCryptoLibByteArray; sigOff: Int32;
  const pk: TCryptoLibByteArray; pkOff: Int32;
  const ctx: TCryptoLibCustomByteArrayBuffer; phflag: Byte;
  const m: TCryptoLibByteArray; mOff, mLen: Int32): Boolean;
var
  r, s, h, k, check,pubkey: TCryptoLibByteArray;
  nS, nA: TCryptoLibUInt32Array;
  pA: TPointAffine;
  pR: TPointAccum;
  d: IDigest;
  checkboolean,checkboolean2:boolean;
  str:string;
begin
  if (not CheckContextVar(ctx, phflag)) then
  begin
  ...
  begin
    result := false;
    writeln(stdout,'CheckScalarVar r fail');
    Exit;
  end;
  ...
  begin
    result := false;
    writeln(stdout,'CheckScalarVar s fail');
    Exit;
  end;
  pA := TPointAffine.CreatePointAffine();
  if (not DecodePointVar(pubkey, pkOff, true, pA)) then
  begin
    result := false;
    writeln(stdout,'DecodePointVar fail');
    Exit;
  end;
  ...
  checkboolean:=EncodePoint(pR, check, 0)<>0;
  str:=THex.Encode(check);
  writeln(StdOut,'Check:',str);
  str:=THex.Encode(r);
  writeln(StdOut,'r:',str);
  writeln(stdout,'EncodePoint=',checkboolean);
  checkboolean2:=TArrayUtils.ConstantTimeAreEqual(check, r);
  writeln(stdout,'ConstantTimeAreEqual=',checkboolean2);
  result := checkboolean and checkboolean2;
end;
end.
