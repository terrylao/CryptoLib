...
type
  ...
  class var
    MAX_SMALL:Integer;
    SMALL_PRIMES_PRODUCT:TBigInteger; // Hexadecimal value of the product of the 131 smallest odd primes from 3 to 743
...
{$ENDIF DELPHI}
    ...
    class function createRandomPrime(bitLength,certainty:integer; random:ISecureRandom):TBigInteger;static;
    class function createRandomBigInteger(bitLength:integer;random:ISecureRandom):TBigInteger;static;
    class function  createRandom(bitLength:integer; random:ISecureRandom):tbytes; static;
    class procedure boot2();static;
    class function createRandomInRange(min,max:TBigInteger;random:ISecureRandom):TBigInteger;static;
    class function modOddInverse( M,  X:TBigInteger):TBigInteger;static;
  end;

implementation

uses
  ClpSecureRandom,ClpNat,ClpMod; // included here to avoid circular dependency :)
...
begin
  ...
  class function  TBigInteger.createRandom(bitLength:integer; random:ISecureRandom):tbytes;
  var
    nBytes,xBits:integer;
    rv:TBytes;
  begin
      if (bitLength < 1) then
      begin
          Raise exception.create('bitLength must be at least 1');
      end;

      nBytes := (bitLength + 7) div 8;
      ...
      rv[0] :=rv[0] and byte(255 shr xBits);

      result := rv;
  end;
  class function TBigInteger.createRandomBigInteger(bitLength:integer;random:ISecureRandom):TBigInteger;
  begin
      result := TBigInteger.create(1, createRandom(bitLength, random));
  end;
  class function TBigInteger.createRandomPrime(bitLength,certainty:integer; random:ISecureRandom):TBigInteger;
  var
    base:TBytes;
    xBits:integer;
    lead:byte;
  begin
      if (bitLength < 2) then
      begin
          Raise exception.create('bitLength < 2');
      end;

      if (bitLength = 2) then
      begin
          if (random.nextInt32() < 0) then
           exit(TWO)
          else
           exit(THREE);
      end;
      ...
      repeat
          base := createRandom(bitLength, random);

          xBits := 8 * length(base) - bitLength;
          lead := byte(1 shl 7 - xBits);

          // ensure top and bottom bit set
          base[0] :=base[0] or lead;
          base[length(base) - 1] :=base[length(base) - 1] or $01;

          result := TBigInteger.create(1, base);
          if (bitLength > MAX_SMALL) then
          begin
              while (not result.gcd(SMALL_PRIMES_PRODUCT).equals(ONE)) do
              begin
                  result := result.add(TWO);
              end;
          end;
      until (result.isProbablePrime(certainty));

  end;
    class function TBigInteger.createRandomInRange(min,max:TBigInteger;random:ISecureRandom):TBigInteger;
    var
      cmp,i:integer;
      x:TBigInteger;
    begin
        ...
        begin
            if (cmp > 0) then
            begin
                Raise Exception.Create('"min" may not be greater than "max"');
            end;
        ...
        begin
            exit(createRandomInRange(GetZero, max.subtract(min), random).add(min));
        end;

        for  i := 0 to MAX_ITERATIONS-1 do
        begin
            x := createRandomBigInteger(max.bitLength, random);
        ...
        result := createRandomBigInteger(max.subtract(min).bitLength - 1, random).add(min);
    end;
    class function  TBigInteger.modOddInverse( M,  X:TBigInteger):TBigInteger;
    var
      bits,len:integer;
    ...
    begin
        ...
        begin
            Raise Exception.Create('"M" must be odd');
        end;
        if (M.Fsign <> 1) then
        begin
            Raise Exception.Create('TBigInteger: modulus not positive');
        end;
        if (X.Fsign < 0) or (X.compareTo(M) >= 0) then
        begin
            X := X.&mod(M);
        end;

        bits := M.bitLength;
        lm  := TNat.fromBigInteger(bits, M);
        lx  := TNat.fromBigInteger(bits, X);
        len := length(lm);
        lz  := TNat.create(len);
        if (0 = TMod.modOddInverse(lm, lx, lz)) then
        begin
            Raise Exception.Create('BigInteger not invertible.');
        end;
        result := TNat.toBigInteger(len, lz);
    end;
...
begin
  ...
  boot2();
    ...
    class procedure TBigInteger.boot2();
    begin
      MAX_SMALL := TBigInteger.valueOf(743).bitLength; // bitlength of 743 * 743
