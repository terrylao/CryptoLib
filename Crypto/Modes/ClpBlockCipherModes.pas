...
type
  ...
  public
    ...
    function getCurrentIV():TCryptoLibByteArray;
    function getCurrentKey():TCryptoLibByteArray;virtual;
...
function TCbcBlockCipher.getCurrentIV():TCryptoLibByteArray;
begin
  setlength(result,length(FIV));
  move(FIV[0],result[0],length(FIV));
end;
function TCbcBlockCipher.getCurrentKey():TCryptoLibByteArray;
begin
  // nothing to do.
end;
procedure TCbcBlockCipher.changeIV(modifier:TCryptoLibByteArray);
begin
  move(modifier[0],FIV[0],length(FIV));
end;
{ TCfbBlockCipher }
...
function TCfbBlockCipher.getCurrentIV():TCryptoLibByteArray;
begin
  setlength(result,length(FIV));
  move(FIV[0],result[0],length(FIV));
end;
function TCfbBlockCipher.getCurrentKey():TCryptoLibByteArray;
begin
  // nothing to do.
end;
procedure TCfbBlockCipher.changeIV(modifier:TCryptoLibByteArray);
begin
  move(modifier[0],FIV[0],length(FIV));
end;
{ TOfbBlockCipher }
...
function TOfbBlockCipher.getCurrentIV():TCryptoLibByteArray;
begin
  result:=FIV;
end;
function TOfbBlockCipher.getCurrentKey():TCryptoLibByteArray;
begin
  // nothing to do.
end;
procedure TOfbBlockCipher.changeIV(modifier:TCryptoLibByteArray);
begin
  // nothing to do.
end;
{ TSicBlockCipher }
...
function TSicBlockCipher.getCurrentIV():TCryptoLibByteArray;
begin
  setlength(result,length(Fcounter));
  move(Fcounter[0],result[0],length(Fcounter));
end;
function TSicBlockCipher.getCurrentKey():TCryptoLibByteArray;
begin
  // nothing to do.
end;
procedure TSicBlockCipher.changeIV(modifier:TCryptoLibByteArray);
var
  J: Int32;
begin
  move(modifier[0],Fcounter[0],length(Fcounter));
  {
  J := System.Length(Fcounter);
  System.Dec(J);
  if modifier>0 then
  begin
    for modifier:=modifier-1  downto 0 do
    begin
      System.Inc(Fcounter[J]);
      while ((J >= 0) and (Fcounter[J] = 0)) do
      begin
  ...
  begin
    for modifier:=modifier+1 to 0 do
    begin
      System.dec(Fcounter[J]);
      while ((J >= 0) and (Fcounter[J] = 0)) do
      begin
        System.Dec(J);
        System.Inc(Fcounter[J]);
