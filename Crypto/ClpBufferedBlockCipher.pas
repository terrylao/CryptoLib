...
begin
  try
    ...
    result := FbufOff;
    Exit;
  except //從final 變成except, final 就是exit 也會被執行
    Reset();
  end;
end;
...
function TBufferedBlockCipher.getCurrentIV():TCryptoLibByteArray;
begin
     result := Fcipher.getCurrentIV();
end;
function TBufferedBlockCipher.getCurrentKey():TCryptoLibByteArray;
begin
     result := Fcipher.getCurrentKey();
end;
procedure TBufferedBlockCipher.changeIV(modifier:TCryptoLibByteArray);
begin
  Fcipher.changeIV(modifier);
end;
end.
