...
type
  ...
  /// </para>
  /// </summary>
  TAesEngine = class sealed(TInterfacedObject, IAesEngine,IBlockCipher) //這個的 IBlockCipher 是必要的，否則程式會有莫明的EXCEPTION而無法起動
  ...
  var
    ...
    FKeyByteArray:TCryptoLibByteArray;
...
begin
  ...
  FKeyByteArray:=keyParameter.GetKey();
...
function TAesEngine.getCurrentIV():TCryptoLibByteArray;
begin
  // nothing to do.
end;
...
function TAesEngine.getCurrentKey():TCryptoLibByteArray;
begin
  result:=FKeyByteArray;
end;
procedure TAesEngine.changeIV(modifier:TCryptoLibByteArray);
begin
  // nothing to do.
end;
end.
